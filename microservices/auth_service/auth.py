...
def utc_from_claim(value: Any) -> datetime:
    ...
    raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid admin token timestamp.")
...
def set_admin_cookie(response: Response, admin_user: dict[str, Any], session_started_at: datetime | None = None) -> None:
...
def clear_admin_cookie(response: Response) -> None:
    secure = use_secure_admin_cookie()
    response.delete_cookie(key=ADMIN_COOKIE_NAME, path=ADMIN_COOKIE_PATH, samesite="lax", secure=secure, httponly=True)
    response.delete_cookie(key=CSRF_COOKIE_NAME, path=CSRF_COOKIE_PATH, samesite="lax", secure=secure, httponly=False)
    response.delete_cookie(key=CSRF_COOKIE_NAME, path=ADMIN_COOKIE_PATH, samesite="lax", secure=secure, httponly=False)


def should_rotate_access_token(payload: dict[str, Any]) -> bool:
    if JWT_ROTATION_INTERVAL_MINUTES <= 0:
        return False
    return datetime.now(timezone.utc) >= utc_from_claim(payload["iat"]) + timedelta(minutes=JWT_ROTATION_INTERVAL_MINUTES)


def rotate_admin_cookie_if_needed(response: Response, admin_user: dict[str, Any], payload: dict[str, Any]) -> None:
    if should_rotate_access_token(payload):
        set_admin_cookie(response, admin_user, session_started_at=get_session_started_at(payload))


def require_admin_user(response: Response, token: str | None = Depends(admin_cookie_scheme)) -> dict[str, Any]:
    if not token:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Missing admin session.")
    payload = decode_access_token(token)
    admin_user = {"id": payload["uid"], "username": payload["sub"]}
