...
def get_oai_identifier(record):
    ...
    if identifier_el is not None and identifier_el.text:
        return identifier_el.text.strip()
...
def parse_dim_metadata(dim_node, oai_identifier):
    ...
    identifiers = (
        dim_field_texts(dim_node, "identifier", "uri")
        + dim_field_texts(dim_node, "identifier")
    )
...
def parse_metadata(metadata, metadata_prefix, oai_identifier):
    ...
    if dc_node is not None:
        return parse_oai_dc_metadata(dc_node, oai_identifier)
