...
- Aggregate report (`report.json`): metadata plus per-query and macro-averaged metrics. `metrics.csv` contains aggregate method rows and `summary.md` is a concise view of the same computed report.
...
After manual judgments have been completed, calculate the report:

```powershell
.\.venv\Scripts\python.exe -m evaluation report --queries path\to\queries.json --judgments path\to\judgments.json --runs path\to\runs.json --output-dir path\to\report --corpus-size 1000 --k 5 10 --embedding-model intfloat/multilingual-e5-large --ranking-config '{"candidate_multiplier":6}' --methods keyword vector_only full_pipeline
```

The report records the current Git commit unless `--git-commit` is supplied, a UTC evaluation timestamp, corpus/query sizes, k values, model identifier, ranking configuration, parser-mode counts, and mean latency when run rows contain latency.
...
4. Have a qualified human assign 0/1/2 judgments under a written relevance rubric. Do not infer missing judgments from ranks or scores.
